...
async function estimateAnnualRewards(cardId: string): Promise<number> {
  try {
    ...
    const { data, error } = await (supabase.from('spending_log') as any)
    ...
    if (data && data.length > 0) {
      return (data as any[]).reduce(
        (sum, entry: any) => sum + (parseFloat(entry.rewards_earned) || 0),
        0
      );
    }
...
function getWorthReason(worth: 'yes' | 'maybe' | 'no', _netValue: number): string {
...
export function getUpcomingRenewals(_days: number = 30): CardFeeAnalysis[] {
