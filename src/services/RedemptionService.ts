...
const transferPartnersCache: Record<string, TransferPartner[]> = {};
...
export async function getTransferPartners(programId: string): Promise<TransferPartner[]> {
  ...
  try {
    if (!supabase) return [];
    const { data, error } = await (supabase.from('transfer_partners') as any)
      .select('*')
...
export async function getRedemptionGuide(programId: string): Promise<ProgramRedemption> {
  try {
    ...
    const { data: programData, error: programError } = await (
      supabase.from('reward_programs') as any
    )
      .select('*')
      .eq('id', programId)
      .single();
    ...
    const { data: optionsData, error: optionsError } = await (
      supabase.from('redemption_options') as any
    )
      .select('*')
      .eq('program_id', programId);
    ...
    const redemptionOptions: RedemptionMethod[] = ((optionsData || []) as any[]).map(
      (row: any) => ({
    ...
    if (pd.optimal_rate_cents) {
      ...
      const best = redemptionOptions.reduce(
        (max, option) => (option.centsPerPoint > max.centsPerPoint ? option : max),
